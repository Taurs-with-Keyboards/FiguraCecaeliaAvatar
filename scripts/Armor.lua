...
local octopus    = sync.add(config:load("ArmorOctopus"), true)
...
for _, trim in ipairs(trims) do
	local tex = textures["textures.armor.trims."..trim.."Trim"] or textures["Cecaelia."..trim.."Trim"] or false
	if tex then
		octopusArmor.TrimPatterns[trim]:setTexture(tex)
	end
end
...
function pings.setArmorAll(boolean)
	...
	config:save("ArmorOctopus", sync[octopus])
...
-- Octopus toggle
function pings.setArmorOctopus(boolean)
...
a.octopusAct = armorPage:newAction()
	:item("ink_sac")
	:toggleItem("glow_ink_sac")
	:onToggle(pings.setArmorOctopus)
...
function events.RENDER(delta, context)
	...
	if action_wheel:isEnabled() then
		...
		a.allAct
			...
			:toggled(sync[helmet] and sync[chestplate] and sync[leggings] and sync[boots] and sync[octopus])
		...
		a.octopusAct
			:title(toJson(
				{
					"",
					{text = "Toggle Octopus Armor\n\n", bold = true, color = c.primary},
					{text = "Toggles visibility of octopus armor.", color = c.secondary}
				}
			))
			:toggled(sync[octopus])
