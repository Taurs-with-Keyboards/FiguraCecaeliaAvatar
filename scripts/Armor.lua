...
local octopus    = sync.new("ArmorOctopus", true):config()
...
function events.RENDER(delta, context)
	...
	for i = 1, #octopusGroups do
		octopusGroups[i]:visible(octopus.curr)
	end
...
acts.armorOctopusToggle = armorPage:newAction()
	:item("ink_sac")
...
function events.RENDER(delta, context)
	...
	if action_wheel:isEnabled() then
		...
		acts.armorAllToggle
			...
			:toggled(helmet.curr and chestplate.curr and leggings.curr and boots.curr and octopus.curr)
			:hoverColor(colors.hover)
		...
		acts.armorOctopusToggle
			:title(toJson(
				{
					"",
					{text = "Toggle Octopus Armor\n\n", bold = true, color = colors.primary},
					{text = "Toggles visibility of octopus armor.", color = colors.secondary}
				}
			))
			:toggled(octopus.curr)
			:hoverColor(colors.hover)
			:toggleColor(colors.active)
